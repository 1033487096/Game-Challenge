    ...
    kill(playerSlain) {
      if (this.state) {
        if (this.history.length === 0) {
          console.log(
            `${this.nickName} a matado al jugador ${playerSlain.nickName} 1`
          );
          ...
          for (let j = 0; j <= this.history.length; j++) {
            if (playerSlain.id === this.history[j].id)
              console.log(
                `El jugador ${this.history[j].id} fue asesinado previamente, no se puede completar la operacion 2`
              );
            else {
              console.log(
                `${this.nickName} a matado al jugador ${playerSlain.nickName}`
              );
              this.history.push(playerSlain);
            }
          }
        ...
        console.log("Este jugador ya fue asesinado, esta fuera del juego.");
    }
  }
